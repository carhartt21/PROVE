#!/bin/bash
# =============================================================================
# Run Missing Stage 1 MapillaryVistas Tests Locally
# =============================================================================
# This script runs fine-grained tests for all 81 MapillaryVistas Stage 1
# configurations that have completed training but are missing test results.
#
# Usage:
#   ./scripts/run_stage1_mapillary_tests.sh              # Run all tests
#   ./scripts/run_stage1_mapillary_tests.sh --dry-run    # Show what would be run
#   ./scripts/run_stage1_mapillary_tests.sh --limit N    # Run only first N tests
#   ./scripts/run_stage1_mapillary_tests.sh --gpu 1      # Use specific GPU
#   ./scripts/run_stage1_mapillary_tests.sh --list-gpus  # List available GPUs
#   ./scripts/run_stage1_mapillary_tests.sh --help       # Show all options
#
# Prerequisites:
#   - CUDA GPU available
#   - 'prove' conda/mamba environment activated
#
# Estimated time: ~10 minutes per test (81 tests total ≈ 13.5 hours)
# =============================================================================
...
LOG_FILE="$PROJECT_ROOT/logs/stage1_mapillary_tests_$(date +%Y%m%d_%H%M%S).log"
...
# Header
echo "============================================================"
echo "Stage 1 MapillaryVistas Fine-Grained Tests"
echo "============================================================"
echo "Time: $(date)"
...
if [ $total_tests -eq 0 ]; then
    echo -e "${GREEN}All MapillaryVistas Stage 1 tests are complete!${NC}"
    exit 0
fi
