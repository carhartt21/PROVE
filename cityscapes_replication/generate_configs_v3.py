...
ALL models use 512x512 crop size as requested.
Models: SegFormer MIT-B3, HRNet HR48, OCRNet HR48, DeepLabV3+ R50, PSPNet R50, SegNeXt MSCAN-B
"""

from pathlib import Path

OUTPUT_DIR = Path('/home/mima2416/repositories/PROVE/cityscapes_replication/configs')
...
def generate_segformer_b3():
    """SegFormer MIT-B3 - Expected mIoU on Cityscapes: ~80%"""
    return f'''"""
SegFormer MIT-B3 Cityscapes config with STANDARD pipeline.
Crop size: 512x512 (smaller than standard 1024x1024 for comparison with PROVE)
"""
...
def generate_ocrnet_hr48():
    """OCRNet with HRNet-W48 backbone"""
    return f'''"""
...
def generate_deeplabv3plus_r50():
    ...
    return f'''"""
DeepLabV3+ with ResNet-50 - Cityscapes with STANDARD pipeline.
Crop size: 512x512
"""
...
def generate_pspnet_r50():
    """PSPNet with ResNet-50"""
    return f'''"""
PSPNet with ResNet-50 - Cityscapes with STANDARD pipeline.
Crop size: 512x512
"""
...
def main():
    ...
    configs = {
        'segformer_mit-b3_cityscapes_512x512.py': generate_segformer_b3(),
        'hrnet_hr48_cityscapes_512x512.py': generate_hrnet_hr48(),
        'ocrnet_hr48_cityscapes_512x512.py': generate_ocrnet_hr48(),
        'deeplabv3plus_r50_cityscapes_512x512.py': generate_deeplabv3plus_r50(),
        'pspnet_r50_cityscapes_512x512.py': generate_pspnet_r50(),
        'segnext_mscan-b_cityscapes_512x512.py': generate_segnext_mscan_b(),
    }
    ...
    print(f"ALL configs use crop_size = {CROP_SIZE}")
    print("\nModel Summary:")
    print("-" * 60)
    print(f"{'Model':<30} {'Iters':<10} {'Crop'}")
    print("-" * 60)
    print(f"{'SegFormer MIT-B3':<30} {'160k':<10} {'512x512'}")
    print(f"{'HRNet HR48':<30} {'160k':<10} {'512x512'}")
    print(f"{'OCRNet HR48':<30} {'160k':<10} {'512x512'}")
    print(f"{'DeepLabV3+ R50':<30} {'80k':<10} {'512x512'}")
    print(f"{'PSPNet R50':<30} {'80k':<10} {'512x512'}")
    print(f"{'SegNeXt MSCAN-B':<30} {'160k':<10} {'512x512'}")
    print("-" * 60)
