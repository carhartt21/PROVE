...
- Created in each model's weights directory (e.g., `/scratch/aaa_exchange/AWARE/WEIGHTS/{strategy}/{dataset}_cd/{model}/.training_lock`)
...
A Python class using `fcntl` file locks, stored in `/scratch/aaa_exchange/AWARE/training_locks/`. These locks are:
...
=== File-based locks (.training_lock) (3) ===
  - gen_VisualCloze/idd-aw/segformer_mit-b5_ratio0p50 [UNKNOWN - remote process]
    ...
    Lock file: /scratch/aaa_exchange/AWARE/WEIGHTS/gen_VisualCloze/idd-aw_cd/segformer_mit-b5_ratio0p50/.training_lock
  - photometric_distort/idd-aw/deeplabv3plus_r50 [ACTIVE]
    PID: 1277413, Started: 2026-01-12T16:52:55.136674
    Lock file: /scratch/aaa_exchange/AWARE/WEIGHTS/photometric_distort/idd-aw_cd/deeplabv3plus_r50/.training_lock
...
```bash
#!/bin/bash
WEIGHTS_PATH="/scratch/aaa_exchange/AWARE/WEIGHTS/{strategy}/{dataset}_cd/{model}"
