...
python fine_grained_test.py \
    ...
    --dataset ACDC \
    --output-dir "${AWARE_DATA_ROOT}/WEIGHTS/domain_adaptation_ablation/baseline/bdd10k/pspnet_r50"

# Rename results to domain_adaptation_evaluation.json
if [ -f "${AWARE_DATA_ROOT}/WEIGHTS/domain_adaptation_ablation/baseline/bdd10k/pspnet_r50/results.json" ]; then
    mv "${AWARE_DATA_ROOT}/WEIGHTS/domain_adaptation_ablation/baseline/bdd10k/pspnet_r50/results.json" "${AWARE_DATA_ROOT}/WEIGHTS/domain_adaptation_ablation/baseline/bdd10k/pspnet_r50/domain_adaptation_evaluation.json"
fi
