...
**Last Updated:** 2026-02-18 (17:15)

---

## 📊 Current Status (2026-02-18 17:15)

### Queue Summary
| User | Category | RUN | PEND | Total |
|------|----------|----:|-----:|------:|
| mima2416 | From-Scratch Ratio=0.0 (40k, resume) | **0** | **78** | **78** |
| mima2416 | From-Scratch std_photometric_distort | **0** | **4** | **4** |
| mima2416 | Extended Ablation Tests (grouped) | **2** | **28** | **30** |
| mima2416 | Other (semantic eval) | **3** | **0** | **3** |
| **Total** | | **5** | **110** | **115** |

**🔄 From-Scratch Ratio=0.0 training (ROUND 3 — corrected max_iters=40k).**
- Previous 80 jobs submitted with 80k max_iters; now corrected to 40k.
- 26/104 gen ratio=0.0 already have `iter_40000.pth` ✅
- 78 remaining resubmitted with `--resume` (Job IDs 4248331–4248778). Many have partial checkpoints (5k–35k from chge7185's parallel submissions) and will resume efficiently.
- 4 new `std_photometric_distort` jobs (Job IDs 4248850–4248853).
- ⚠️ **chge7185 interference:** Another user submitted parallel from-scratch jobs with their own code (unfixed Pad bug, max_iters=40000). Their jobs crashed during validation but left partial checkpoints (5k–40k) and stale locks. Locks cleaned; our jobs resume from their checkpoints.

**🔄 Extended Training Ablation Tests (SUBMITTED).**
- 434 checkpoints across WEIGHTS_EXTENDED_ABLATION grouped into 30 LSF jobs.
- Script: `scripts/submit_extended_ablation_tests.py` (rewritten for grouped testing).
- Job IDs 4241546–4241575. 2 running, 28 pending.

**✅ Broken Pipeline Retrain — ROUND 2 COMPLETE.**
- S1 PSPNet BDD10k + 4 S2 baseline retrains all completed.

**✅ Loss Ablation Tests — COMPLETE.**
- All 18 test results validated. Results in `WEIGHTS_LOSS_ABLATION/`.

### ⚠️ Known Issues & Open Questions (2026-02-18)

1. **chge7185 parallel submissions created mixed-ownership checkpoints.**
   - Another user submitted from-scratch jobs with their own conda env (`/home/chge7185/.local/share/mamba/envs/prove/`), using `max_iters=40000` and **unfixed code** (missing Pad transform).
   - Their jobs crashed during validation (Pad bug) but left partial checkpoints (5k–40k) owned by `chge7185`.
   - Our resume jobs now resume from their checkpoints. This should be fine since PyTorch checkpoints contain model state only, not code paths. However, the `training_config.py` files in those directories were generated by chge7185's code — our resumed jobs will overwrite with correct configs.
   - **Question:** Should we verify that resumed models produce expected mIoU (no data corruption from the mixed checkpoint chain)?

2. **From-scratch training now at 40k (corrected from 80k).**
   - Previous code incorrectly set `max_iters=80000` for `from-scratch` stage. Corrected to 40k (commit `4395d1d`).
   - 29 existing ratio=0.5 jobs already reached 80k — these have extra checkpoints beyond 40k. The iter_40000.pth is the official evaluation point.
   - **Question:** Should we clean up the 80k checkpoints from ratio=0.5 dirs to save disk space, or keep them for potential extended analysis?

3. **`WEIGHTS_EXTENDED` (legacy 69GB) was deleted** — stale results referencing this directory exist in `result_figures/extended_training/` within the PROVE repo. These figures should be regenerated from `WEIGHTS_EXTENDED_ABLATION/` once the 434 test jobs complete.

4. **Loss ablation results not yet exported to IEEE repo.** The 18 validated loss ablation test results need to be exported as `loss_ablation_results.csv` to the IEEE publication repository.

5. **Extended ablation test results pending.** 30 grouped test jobs (434 checkpoints) submitted. After completion: regenerate learning curve figures and export to IEEE repo.

6. **`batch_training_submission.py` does not check LSF queue for duplicates.** It relies solely on training locks. This means resubmitting with `--resume` can create duplicate queue entries for already-queued jobs. The locks prevent actual duplicate training, but wasted queue slots increase wait times. Consider adding `bjobs` integration for duplicate detection.
...
| From-Scratch Ratio=0.0 (40k) | **26/104** | 78 | 0 | 🔄 Resubmitted (round 3, corrected 40k) |
...
| Loss Ablation | **18/18** | ~20 | ✅ All validated |
| Extended Ablation (multi-ckpt) | **0/434** | 30 | 🔄 30 grouped test jobs submitted |
...
11. ✅ **Loss Ablation Testing** — All 18 test results validated (2026-02-18). 3 models × 2 loss variants × baseline + gen_stargan_v2 + gen_step1x_new × 2 stages. Results in `WEIGHTS_LOSS_ABLATION/`. **TODO: export to IEEE repo as loss_ablation_results.csv.**
12. 🔄 **From-Scratch Ratio=0.0 (100% Generated)** — Round 3 resubmitted (2026-02-18 17:04). **Corrected: max_iters=40k** (was incorrectly set to 80k in previous rounds). 26/104 already complete at 40k; 78 remaining jobs submitted with `--resume`. 20 gen_* strategies × 4 datasets × 1 model (segformer_mit-b3). Job IDs 4248331–4248778. After completion: compare vs ratio=0.50 and baseline results.
13. 🔄 **Extended Training Ablation Testing** — 434 checkpoints across 30 grouped test jobs submitted (2026-02-18 16:36). Tests all intermediate checkpoints in WEIGHTS_EXTENDED_ABLATION (stage1: 15k→45k, cityscapes_gen: 20k→60k). Job IDs 4241546–4241575. After completion: regenerate comprehensive learning curve figures.
